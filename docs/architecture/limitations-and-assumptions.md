...
proyecto: _Configuration first, customization only when justified_.
...
| ------------ | -------------------------------------------------------------------------------------- | ------------------------------- | ---------------------------------- | ------ |
| CON-ARCH-001 | Respetar _Configuration first, customization only when justified_                      | Mantenibilidad y upgradeability | `standard-vs-custom-framework.md`  | Activa |
...
| RISK-016 | _Skip Entitlement Checks During Search_ está **ON** en la tienda: la búsqueda no revalida entitlements por buyer. Hoy es inocuo (política _All Access_), pero **dispara si algún día hay catálogo restringido**: productos no entitled podrían aparecer en búsqueda/facets | Seguridad/Search | Baja (hoy) / Alta (con catálogo restringido) | Alto    | Al introducir catálogo restringido: desactivar el skip, reindexar y re-testear visibilidad como buyer (REGLA-001/003) | Abierto — revisar antes de la fase de catálogo restringido |
