...
> _Confirmadas_ (son decisiones de proyecto). Las decisiones de **configuración
> técnica** que dependen de la org se marcan como _Pendiente de validación_ o
> _Supuesto_; no se inventa configuración concreta.
...
| CFG-GEN-002 | Mantener _Configuration first, customization only when justified_ | Confirmada                 | Principio rector                | `PROJECT_CONTEXT.md`              | No                     |
...
| ----------- | ------------------------------------------------------------------------------------------------------------------------- | ---------------------- | -------------------------------------------------------------------------------------------------- | ---------------------------- |
| CFG-PRD-001 | Usar productos estándar (Product2) cuando aplique                                                                         | Confirmada             | Validado en la org (catálogo `LVL-*` operativo)                                                    | No                           |
...
| CFG-PRD-006 | Campos de contenido transversales en Product2 (`Brand__c`, `Warranty_Months__c`, `EAN__c`)                                | Confirmada             | Gap validado (V4); fase 1 de enriquecimiento                                                       | Sí — `adr/0008` (Accepted)   |
| CFG-PRD-007 | `Family` poblado con la taxonomía oficial de 8 categorías (Name de categoría = valor de picklist)                         | Confirmada             | Seed idempotente sobre internos activos; Platzi excluido (fase 4)                                  | No (cubierto por `adr/0008`) |
| CFG-PRD-008 | `Brand__c` como picklist restringido (no Text)                                                                            | Confirmada             | Los facets de Commerce solo aceptan tipos enumerables; Text→Picklist no es convertible en esta org | No (cubierto por `adr/0008`) |
| CFG-PRD-009 | `Description` con HTML estructurado por plantilla fija (párrafo B2B + specs + MOQ/múltiplo desde los campos del registro) | Confirmada             | Renderiza en el acordeón estándar de la PDP; una sola fuente con Quick Buy                         | No (cubierto por `adr/0008`) |
...
| CFG-PLP-007 | Heading de la PDP con mappings Marca/Familia/Garantía (límite estándar: 3 mappings; Product Code eliminado por redundante con el SKU) | Confirmada                           | Validado en Builder + buyer real (fase 1)                             | No (cubierto por `adr/0008`) |
| CFG-PLP-008 | Línea de marca en las cards de categoría y búsqueda (`Brand__c`, sin label)                                                           | Confirmada                           | `cardContentMapping` estándar; requiere FLS de lectura del buyer      | No (cubierto por `adr/0008`) |
| CFG-PLP-009 | Search: facets Familia/Marca, sort por precio (Price Book for Sorting = LevelUp Wholesale Price Book) y Partial SKU Search            | Confirmada                           | Configuración estándar de la app Commerce; un único reindex por tanda | No (cubierto por `adr/0008`) |
...
| CFG-SEC-009 | El canal commerce exige FLS de lectura en campos custom de Product2 (field mappings de PDP y campos de card); se resuelve con permission sets por feature (`LvlUp_Product_Content_Admin` / `LvlUp_Product_Content_Buyer`) | Confirmada              | Validación V6 (fase 1): Family (estándar) renderizaba y los custom no, hasta conceder lectura al buyer | No (cubierto por `adr/0008`) |
