...
| `docs/salesforce/product-content-enrichment-runbook.md` | Runbook con la receta replicable de enriquecimiento de contenido de producto por categoría (campos de contenido, seed idempotente en 2 lotes, modelo de marca, reindex único, QA como buyer) + validaciones V4/V5/V6 y lecciones de plataforma. Fase 3 (2026-07-27): 9 categorías curadas | Creado |
...
| `adr/0009-purchase-quantity-rule-migration.md`            | Migración de las reglas de cantidad de compra (MOQ/múltiplo/máximo) del campo custom al objeto estándar `PurchaseQuantityRule` (junction `ProductQuantityRule`), con Quick Buy sin cambios | Accepted |
